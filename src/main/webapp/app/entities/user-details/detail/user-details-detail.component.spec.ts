...
describe('UserDetails Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./user-details-detail.component').then(m => m.UserDetailsDetailComponent),
              resolve: { userDetails: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load userDetails on init', async () => {
      ...
      // THEN
      expect(instance.userDetails()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
