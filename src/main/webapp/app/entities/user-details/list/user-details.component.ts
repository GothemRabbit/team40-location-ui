...
import { EntityArrayResponseType, UserDetailsService } from '../service/user-details.service';
...
export class UserDetailsComponent implements OnInit {
  ...
  trackId = (item: IUserDetails): number => this.userDetailsService.getUserDetailsIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userDetails = this.refineData(dataFromBody);
  }

  protected refineData(data: IUserDetails[]): IUserDetails[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IUserDetails[] | null): IUserDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.userDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
